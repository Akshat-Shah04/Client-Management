document.addEventListener("DOMContentLoaded", function () {
  ...
  form.addEventListener("submit", function (event) {
      ...
      serviceCheckboxes.forEach(function (checkbox) {
          ...
          if (checkbox.checked) {
              // Get the values for fee, status, billing date, and description
              ...
              const billingDateValue = document.querySelector(`input[name="billing_date_${serviceId}"]`).value;
              const descValue = document.querySelector(`input[name="desc_${serviceId}"]`).value;

              // Validate fee, status, and description (since billing date is optional)
              if (!feeValue || !statusValue || !descValue) {
                  valid = false;
                  alert("Please fill out all required fields (Fee, Status, and Description).");
              }
          }
      });
